      ...
      vcode(options = {}) {
        return {
          validator: (rule, value, callback) => {
            if (!/^\d{6}$/.test(value)) {
              callback(new Error(makeMsg('请输入正确的六位数字验证码', options.extraMsg)))
            } else {
              callback()
            }
          },
          ...
          validator: (rule, value, callback) => {
            if (!value) {
              ...
              if (!/^1\d{10}$/.test(value)) {
                callback(new Error(makeMsg('请输入正确的手机号码', options.extraMsg)))
              } else {
                callback()
              }
            }
        ...
        return {
          validator: (rule, value, callback) => {
            if (value.length < 8 || value.length > 20) {
              callback(new Error(makeMsg('密码长度须在8-20之间', options.extraMsg)))
            } else if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/.test(value)) {
              callback()
            } else {
              callback(new Error(makeMsg('密码须包括大小写字母和数字，不能包含特殊字符', options.extraMsg)))
            }
          },
          trigger: 'blur'
        }
          ...
          validator: (rule, value, callback) => {
            ...
            if (!value) {
              ...
              if (reg.test(value)) {
                ...
                callback(new Error(makeMsg('请输入正确的电话号码', options.extraMsg)))
              }
            }
          },
          trigger: 'blur'
      ...
      positiveInteger(options = {}) {
        return {
          validator: (rule, value, callback) => {
            if (value === undefined || value === '') {
              callback()
            } else if ((Number.isInteger(Number(value)) && Number(value) > 0) || !value) {
              callback()
            } else {
        ...
        return {
          validator: (rule, value, callback) => {
            if (value === '' || _.isNaN(Number(value))) {
              ...
              callback(new Error(`不能小于${minValue}。` + (options.errorMsg || '')))
            } else if (options.canEqual === false && Number(value) == Number(minValue)) {
              callback(new Error(`不能小于或等于${minValue}。` + (options.errorMsg || '')))
            } else {
              callback()
            }
          },
        ...
        return {
          validator: (rule, value, callback) => {
            if (value === '' || _.isNaN(Number(value))) {
              ...
              callback(new Error(`不能大于${maxValue}。` + (options.errorMsg || '')))
            } else if (options.canEqual === false && Number(value) == Number(maxValue)) {
              callback(new Error(`不能大于或等于${maxValue}。` + (options.errorMsg || '')))
            } else {
              callback()
            }
          },
